...
describe BackfillStoreProjectFullPathInRepo, :migration do
  ...
  describe '#up' do
    shared_examples_for 'writes the full path to git config' do
      it 'writes the git config', :sidekiq_inline_tech_debt do
      ...
      it 'retries in case of failure', :sidekiq_inline_tech_debt do
      ...
      it 'cleans up repository before writing the config', :sidekiq_inline_tech_debt do
        expect_next_instance_of(Gitlab::GitalyClient::RepositoryService) do |repository_service|
  ...
  describe '#down' do
    context 'project in group' do
      ...
      it 'deletes the gitlab full config value', :sidekiq_inline_tech_debt do
