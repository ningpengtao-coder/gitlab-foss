  ...
  describe 'GET /projects/:id/artifacts/:ref_name/download?job=name' do
    ...
    context 'find proper job' do
      shared_examples 'a valid file' do
        context 'when artifacts are stored locally', :sidekiq_inline_tech_debt do
          let(:download_headers) do
            { 'Content-Transfer-Encoding' => 'binary',
              'Content-Disposition' =>
    ...
    context 'when job has artifacts' do
      ...
      context 'when user is anonymous' do
        ...
        context 'when project is public' do
          ...
          it 'allows to access artifacts', :sidekiq_inline_tech_debt do
            expect(response).to have_gitlab_http_status(200)
            expect(response.headers.to_h)
              .to include('Content-Type' => 'application/json',
                          'Gitlab-Workhorse-Send-Data' => /artifacts-entry/)
      ...
      context 'when user is authorized' do
        ...
        it 'returns a specific artifact file for a valid path', :sidekiq_inline_tech_debt do
          expect(Gitlab::Workhorse)
            .to receive(:send_artifacts_entry)
                  .and_call_original
