...
describe MergeRequests::CreateFromIssueService do
  ...
  describe '#execute' do
    shared_examples_for 'a service that creates a merge request from an issue' do
      ...
      it 'creates a branch based on issue title', :sidekiq_inline_tech_debt do
        service.execute

        expect(target_project.repository.branch_exists?(issue.to_branch_name)).to be_truthy
      end

      it 'creates a branch using passed name', :sidekiq_inline_tech_debt do
      ...
      it 'creates the new_merge_request system note', :sidekiq_inline_tech_debt do
        expect(SystemNoteService).to receive(:new_merge_request).with(issue, project, user, instance_of(MergeRequest))

        service.execute
      end

      it 'creates the new_issue_branch system note when the branch could be created but the merge_request cannot be created', :sidekiq_inline_tech_debt do
      ...
      it 'creates a merge request', :sidekiq_inline_tech_debt do
        expect { service.execute }.to change(target_project.merge_requests, :count).by(1)
      end

      it 'sets the merge request author to current user', :sidekiq_inline_tech_debt do
        result = service.execute

        expect(result[:merge_request].author).to eq(user)
      end

      it 'sets the merge request source branch to the new issue branch', :sidekiq_inline_tech_debt do
        result = service.execute

        expect(result[:merge_request].source_branch).to eq(issue.to_branch_name)
      end

      it 'sets the merge request source branch to the passed branch name', :sidekiq_inline_tech_debt do
        result = service_with_custom_source_branch.execute

        expect(result[:merge_request].source_branch).to eq(custom_source_branch)
      end

      it 'sets the merge request target branch to the project default branch', :sidekiq_inline_tech_debt do
        result = service.execute

        expect(result[:merge_request].target_branch).to eq(target_project.default_branch)
      end

      it 'executes quick actions if the build service sets them in the description', :sidekiq_inline_tech_debt do
      ...
      context 'when ref branch is set', :sidekiq_inline_tech_debt do
    ...
    context 'target_project_id is specified' do
      ...
      context 'target project is a fork of project project' do
        ...
        it 'sets the merge request title to: "WIP: $issue-branch-name', :sidekiq_inline_tech_debt do
          result = service.execute
