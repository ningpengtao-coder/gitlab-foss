...
RSpec.describe Gitlab::Gpg::InvalidGpgSignatureUpdater do
  describe '#run' do
    ...
    context 'gpg signature did have an associated gpg key which was removed later' do
      ...
      it 'assigns the gpg key to the signature when the missing gpg key is added', :sidekiq_inline_tech_debt do
    ...
    context 'gpg signature did not have an associated gpg key' do
      ...
      it 'updates the signature to being valid when the missing gpg key is added', :sidekiq_inline_tech_debt do
        # InvalidGpgSignatureUpdater is called by the after_create hook
        gpg_key = create :gpg_key,
          key: GpgHelpers::User1.public_key,
          user: user
    ...
    context 'gpg signature did have an associated unverified gpg key' do
      ...
      it 'updates the signature to being valid when the user updates the email address', :sidekiq_inline_tech_debt do
        gpg_key = create :gpg_key,
          key: GpgHelpers::User1.public_key,
          user: user
      ...
      it 'keeps the signature at being invalid when the changed email address is still unrelated', :sidekiq_inline_tech_debt do
        gpg_key = create :gpg_key,
          key: GpgHelpers::User1.public_key,
          user: user
