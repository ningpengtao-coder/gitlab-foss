...
describe 'User squashes a merge request', :js do
  ...
  shared_examples 'squash' do
    it 'squashes the commits into a single commit, and adds a merge commit', :sidekiq_inline_tech_debt do
  ...
  shared_examples 'no squash' do
    it 'accepts the merge request without squashing', :sidekiq_inline_tech_debt do
      expect(page).to have_content('Merged')
