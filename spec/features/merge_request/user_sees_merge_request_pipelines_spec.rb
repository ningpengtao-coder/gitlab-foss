...
describe 'Merge request > User sees pipelines triggered by merge request', :js do
  ...
  context 'when a user created a merge request in the parent project' do
    ...
    it 'sees the latest detached merge request pipeline as the head pipeline', :sidekiq_inline_tech_debt do
      page.within('.ci-widget-content') do
        expect(page).to have_content("##{detached_merge_request_pipeline.id}")
      end
    end

    context 'when a user updated a merge request in the parent project', :sidekiq_inline_tech_debt do
    ...
    context 'when a user merges a merge request in the parent project', :sidekiq_inline_tech_debt do
      before do
    ...
    context 'when there are no `merge_requests` keyword in .gitlab-ci.yml' do
      ...
      it 'sees the latest branch pipeline as the head pipeline', :sidekiq_inline_tech_debt do
        page.within('.ci-widget-content') do
  ...
  context 'when a user created a merge request from a forked project to the parent project', :sidekiq_inline_tech_debt do
    let(:merge_request) do
      create(:merge_request,
